...
- MVP currently does **not** yet have:
  ...
  - Rancher Desktop installation path
...
- `[~]` Default path one click (`Set up automatically`)
  - Implemented semantics in Settings/start flows, but true one-click installer path is not integrated yet.
...
- `[~]` Provisioning idempotent/resumable/reboot-safe
  - Checkpoints, run status, retry backoff, and stage-resume from checkpoint exist; reboot-safe resume execution still pending.
...
- `[~]` Host Engine semantics defined
  - Detect/connect existing endpoint is present; Rancher Desktop install flow not yet implemented.
...
- `[~]` Reboot-required handling + resume execution
  - Reboot-required failures are classified and persisted; app startup now calls resume provisioning when checkpoint exists.
...
- `[~]` Install Host Engine action
  - Runs host-specific provisioning stages (detect/validate/health); Rancher Desktop UI install is policy-disabled.
- `[x]` Switch active engine
- `[x]` Repair active engine
- `[x]` Remove managed engine (config-level)
- `[x]` Provisioning progress card with stage-level statuses and retry

## 6) Host Engine Plan (Phase 2)

- `[ ]` Rancher Desktop install via official channel/winget
  - Policy note: do not implement Rancher Desktop UI install path.
- `[ ]` Configure/verify Moby mode
...
- `[~]` Provider installed/switched/repaired event telemetry
  - Switch/rollback/repair + reconnect + provisioning events are emitted; install-source breadth is still incomplete.
- `[~]` Local log paths for bootstrapper/helper/reconnect flow
  - Local JSONL engine event log and relay lifecycle metadata are now persisted in app data logs/state.
...
- `[~]` Route provisioning commands through helper instead of app process
  - Stage runner now routes privileged operations via centralized `run_privileged_action` dispatch and attempts `docker-gui-provisioning-helper` first, with logged in-process fallback when helper transport is unavailable.
- `[~]` Add consent/elevation UX for install/repair actions
  - Settings and guided repair flows now require explicit user confirmation before WSL privileged install/repair/retry commands are invoked; helper-driven UAC/elevation handoff is still pending.
...
- `[~]` Implement relay registration and lifecycle
  - Registration + health/lifecycle state tracking are implemented; relay process start/ensure-running automation is pending.
...
- `[ ]` Implement Rancher Desktop install adapter
